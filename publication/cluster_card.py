...
def create_card(cluster_id, df_table, df_clustering, df_effects, df_pvals, output_dir):
    ...
    ax_main.add_patch(
        mpatches.Rectangle((0.15, 13.15), 9.7, 0.15, facecolor=color, edgecolor="none")
    )
    ...
    # Background
    ax_phate.scatter(
        df_clustering["PHATE_0"],
        df_clustering["PHATE_1"],
        c=COLOR_BG_POINTS,
        s=0.3,
        alpha=0.4,
        rasterized=True,
    ...
    ax_phate.scatter(
        cluster_phate["PHATE_0"],
        cluster_phate["PHATE_1"],
        c=color,
        s=6,
        alpha=0.9,
        linewidths=0,
    ...
    ax_volcano.scatter(
        cluster_volcano["effect_size"],
        cluster_volcano["neg_log10_pval"],
        c=color,
        s=8,
        alpha=0.9,
        linewidths=0,
    ...
    ax_volcano.set_xlabel(
        "Effect size", fontsize=FONT_AXIS_LABEL, color=COLOR_TEXT_SECONDARY, labelpad=2
    )
    ...
    if pd.notna(cluster_summary):
        summary_text = (
            cluster_summary[:400] + "..."
            if len(str(cluster_summary)) > 400
            else str(cluster_summary)
        )
        wrapped = textwrap.fill(summary_text, width=100)
        for line in wrapped.split("\n")[:5]:
            ax_main.text(
                0.6, y, line, fontsize=5.5, color=COLOR_TEXT_SECONDARY, style="italic"
            )
            y -= 0.22
    y -= 0.25
    ...
    ax_main.text(
        5, 0.4, footer, fontsize=5.5, color=COLOR_TEXT_MUTED, ha="center", va="center"
    )

    # Adjust y limits
    ax_main.set_ylim(max(0, min(y - 0.3, 0.1)), 14)

    # Save as PNG (600 dpi), PDF, and SVG
...
def _style_subplot(ax, title):
    ...
    ax.tick_params(
        axis="both",
        labelsize=FONT_AXIS_TICK,
        colors=COLOR_TEXT_MUTED,
        length=2,
        width=0.5,
        pad=1,
    )
